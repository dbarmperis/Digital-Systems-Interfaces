...
<span>https://bitbucket.org/teckel12/arduino-new-ping/downloads</span>
.
...
τροφοδοσία 5V (Vcc) και γείωση (GND). Τα δύο πρώτα συνδέονται στην τάση 5 V και GND αντίστοιχα, ενώ
το VIN συνδέεται στο Digital pin 9 του Arduino.

O **buzzer** έχει ένα control pin που όταν δεχθεί μηδενική τάση παράγει
ήχο, τροφοδοσία (Vcc τοποθετούμε 5V) και γείωση (GND). Το control pin
...
καθώς και ένα εκ των δύο DATA. Το VCC συνδέεται σε θύρα τάσης 5V. Το GND
στη γείωση. Τέλος, το DATA pin συνδέεται στο όγδοο Digital pin (D8).
...
πολύ φθηνών εξαρτημάτων εγκυμονεί τον κίνδυνο να μην ανταποκρίνονται στα
